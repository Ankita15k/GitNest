...
function DiffChunk({ file }) {
  ...
  return (
    <div className="rounded-xl border border-zinc-200 dark:border-white/10 overflow-hidden mb-4">
      <button
        type="button"
        className="w-full flex items-center justify-between px-4 py-2.5 bg-zinc-100 dark:bg-white/[0.03] border-b border-zinc-200 dark:border-white/10"
              ...
              {(file.chunks || []).map((chunk, i) => (
                ...
                >
                  ...
                  <td className="w-6 px-2 py-1 text-center select-none">
                    <span className={chunk.type === 'added' ? 'text-emerald-400' : chunk.type === 'removed' ? 'text-red-400' : 'text-zinc-600'}>
  ...
  return (
    <div className="flex gap-3 py-4 border-b border-zinc-200 dark:border-white/5 last:border-0">
      <div className="w-8 h-8 rounded-full bg-emerald-400/20 border border-emerald-400/20 flex items-center justify-center text-emerald-400 font-bold text-sm flex-shrink-0">
        {getApiUserAvatar(comment.author)}
      </div>
...
export default function PullRequestDetailPage() {
  ...
  const invalidatePullRequest = async () => {
  ...
  const commentMutation = useMutation({
    mutationFn: () => addPullRequestComment(id, comment.trim()),
    onSuccess: async () => {
      setComment('');
  ...
  const reviewMutation = useMutation({
    mutationFn: (action) => submitPullRequestReview(id, action),
    onSuccess: invalidatePullRequest,
  });
  ...
  if (isLoading) {
    return <div className="min-h-screen grid place-items-center bg-white dark:bg-[#06070a]"><Spinner /></div>;
  }

  if (isError || !pr) {
    return (
      <div className="min-h-screen bg-white dark:bg-[#06070a] p-6">
        <div className="max-w-3xl mx-auto">
          <ErrorState message={error?.message || 'Could not load pull request.'} />
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
        ...
        <div className="mb-6">
          ...
          <div className="flex flex-wrap items-center gap-3 text-sm text-zinc-500">
            ...
            <span className="flex items-center gap-1.5">
              ...
              <code className="px-1.5 py-0.5 rounded bg-zinc-100 dark:bg-white/5 text-xs font-mono">{pr.fromBranch || pr.sourceBranch}</code>
              <span>to</span>
              <code className="px-1.5 py-0.5 rounded bg-zinc-100 dark:bg-white/5 text-xs font-mono">{pr.toBranch || pr.targetBranch}</code>
            </span>
            <span className="flex items-center gap-1.5">
          ...
          <div className="space-y-6">
            <div className="rounded-2xl border border-zinc-200 dark:border-white/10 overflow-hidden">
              <div className="flex items-center gap-2 px-5 py-3 bg-zinc-50 dark:bg-white/[0.02] border-b border-zinc-200 dark:border-white/10">
                <div className="w-7 h-7 rounded-full bg-emerald-400/20 flex items-center justify-center text-emerald-400 font-bold text-xs">
                  {getApiUserAvatar(pr.author)}
                </div>
                <span className="font-semibold text-sm">{author}</span>
                <span className="text-xs text-zinc-500">created {getTimeAgo(pr.createdAt)}</span>
              </div>
            ...
            <div className="rounded-2xl border border-zinc-200 dark:border-white/10 overflow-hidden">
              ...
              <div className="divide-y divide-zinc-200 dark:divide-white/5 px-5">
                {comments.map((item) => <CommentItem key={item._id || item.id} comment={item} />)}
              </div>
              ...
              <div className="px-5 py-4 border-t border-zinc-200 dark:border-white/10 bg-zinc-50 dark:bg-white/[0.01]">
                <textarea
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/[0.03] text-sm text-zinc-900 dark:text-white placeholder-zinc-500 outline-none focus:border-emerald-400/50 resize-none transition-colors disabled:opacity-60"
                />
        ...
        {activeTab === 'diff' && (
          <div>
            ...
            {(pr.diff || []).map((file) => <DiffChunk key={file.file} file={file} />)}
          </div>
        )}
      </div>
    </div>
